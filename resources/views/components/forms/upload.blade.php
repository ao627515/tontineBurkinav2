...
<div{!! $group['attrs'] !!}>
    ...
    @isset($grid[1])
        ...
        <div class="custom-file">
            <input value="{{ request()->input($attr['name'], old($attr['name'])) }}" {!! $attr !!}
                id="upload-{{ $attr['name'] }}" type="file" aria-describedby="help-{{ $attr['name'] }}">
            <label class="custom-file-label" for="upload-{{ $attr['name'] }}">{{ $placeholder }}</label>

            @if (!empty($help))
                <small id="help-{{ $attr['name'] }}" class="form-text text-muted">We'll never share your email with
                    anyone else.</small>
            @endif
